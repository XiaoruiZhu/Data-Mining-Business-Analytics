...
`summary()` is a function that calculate simple statistics for a given vector. 
...
Element-wise operations for single vector or vectors:
...
## Basic Plotting {.tabset .tabset-fade .tabset-pills}

### `plot()` for scatterplot {.tabset .tabset-fade .tabset-pills}
...
```{r chunk14}
# simulate 100 numbers from the standard normal distribution
x <- rnorm(100)
# simulate 100 numbers from the standard normal distribution
y <- rnorm(100)

# Scatter plot using x and y 
plot(x, y)
# Scatter plot with more clear caption and labels
plot(x, y, xlab = "this is the x-axis",
    ylab = "this is the y-axis",
    main = "Plot of X vs Y")
```

### `hist()` for histogram {.tabset .tabset-fade .tabset-pills}

The `hist()` function is the way to display the histogram of a sequence of numbers in `R`. For example, the `x` vector/series we simulated, we can display its histogram as simple as follows: 
...
### `boxplot()` for scatterplot {.tabset .tabset-fade .tabset-pills}

The `boxplot()` function is the way to draw the boxplot.
...
### NO! 
You may face some challenges in the rest of the semester..., so please learn these basic syntaxes and get used to them!
